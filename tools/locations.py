...
from .async_httpx import AsyncHttpx


@tool
def get_location_coordinate_bak(
...
@tool
async def get_location_coordinate(
        location: Annotated[str, "要获取经纬度的地点名称"],
        city: Annotated[str, "要获取经纬度的地点所在的城市名称，必须是地级市，不能是县级市或村镇。如果能确定的话填写，不能确定可以不填"] = ""
) -> List[Dict[str, str]]:
    """位置获取工具。根据地点名称和城市名称获取该地点的经纬度。由于可能存在同名地点，所以返回的是一个包含所有同名地点详细地址和经纬度的列表"""
...
import asyncio
async def main():
    a = await get_location_coordinate.ainvoke({"location": "岳阳楼", "city": "岳阳"})
    print(a)
    return a
...
if __name__ == "__main__":
    ...
    # a = get_location_coordinate.invoke({"location": "岳阳楼", "city": "岳阳"})
    # print(a)
    asyncio.run(main())
